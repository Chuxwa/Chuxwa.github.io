...
window.paperRenderer = (function() {
    ...
    function renderHeader() {
        ...
        authorsContainer.innerHTML = authors.map(author => {
            ...
            const authorName = `${author.name}${authorMarks}${affiliationSups}`;
            
            if (author.url) {
                return `<a href="${author.url}" class="author">${authorName}</a>`;
            } else {
        ...
        let affiliationsHTML = Object.entries(affiliations).map(([key, affiliation]) => {
            if (affiliation.url) {
        ...
        linksContainer.innerHTML = Object.values(links).map(link => 
            `<a href="${link.url}" class="btn-paper">${link.icon} ${link.text}</a>`
        ).join('\n');
    }
