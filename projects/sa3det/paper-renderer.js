...
window.paperRenderer = (function() {
    ...
    function renderHeader() {
        ...
        // 添加标记说明
        const hasFirstStudent = authors.some(author => author.isFirstStudent);
        const hasCorrespondingAuthor = authors.some(author => author.isCorrespondingAuthor);
        
        if (hasFirstStudent || hasCorrespondingAuthor) {
            const notesDiv = document.createElement('div');
            notesDiv.className = 'author-notes';
            notesDiv.style.cssText = 'margin-top: 10px; font-size: 0.9em; color: #666;';
            if (hasFirstStudent) {
                const span = document.createElement('span');
                span.style.cssText = 'color: #ED764A; font-weight: bold;';
                span.textContent = '+';
                notesDiv.appendChild(span);
                notesDiv.appendChild(document.createTextNode(' First Student Author'));
            }
            if (hasFirstStudent && hasCorrespondingAuthor) {
            ...
            if (hasCorrespondingAuthor) {
                ...
                span.textContent = '*';
                notesDiv.appendChild(span);
                notesDiv.appendChild(document.createTextNode(' Corresponding Author'));
            }
            affiliationsContainer.appendChild(notesDiv);
        }

        // 论文链接
        const linksContainer = document.getElementById('paper-links');
        linksContainer.innerHTML = '';
        Object.values(links).forEach(link => {
            const a = document.createElement('a');
            a.href = sanitizeUrl(link.url);
            a.className = 'btn-paper';
